...
def main():
    ...
    def input_pwd(s, len_=0):
        """
        for user to choose whether use the `input_pwd` method
        which can hide password while typing.
        """
        if hide_pwd:
    ...
    def quit_():
        ...
        if not Diary.saved and input('Not saved. Save? <y/n>') == 'y':
            # save the file
            Diary.save()
        ...
        with open('files/config.ini', 'w') as cf:
            config.write(cf)
            # write the configuration file, including the default diary book.
    ...
    def view_():
        ...
        if get_int(
            ...
            Diary.search(keyword).view_page()
            # select those with specified keyword in it
        print('======================')
        return None
    ...
    def sync_():
        ...
        key = input_pwd('Please input your authorization code: ')
        if get_int(
                'Choose mode:\n1. Sync to the mailbox\n'
                '2. Sync from the mailbox\n#: ',
                range_=(1, 2)
        ) == 1:
            send(key, Diary.current_file, note_='Diary_Sync')
            # send the current file to your mailbox
        else:
            for each in receive(key, input('Specify date (yy-mm-dd-hh-mm): ')):
                if each.note == 'Diary_Sync' and \
                        each.name == Diary.current_file:  # is a diary file
                    each.update()  # save the file to local
                    print(
                        '---- Synchronized "%s" successfully ----' % each.name
                    )
    ...
    def import_():
        ...
        index = get_int(
            'Import (input the #, if you want to create a new one, input 0): ',
            range_=range(_+1)
        )
        if index == 0:  # create a new book
            filename = 'files/' + input('Name your new diary book (.dr): ')
            Diary.change_file(filename).set_password(
                input_pwd('New password: ', len_=8)
            ).init()
        else:  # import the selected one
            new_password = input_pwd(
                'Password for %s: ' % diaries[index-1], len_=8
            )
    ...
    def recall_():
        ...
        """
        keywords = input(
            'Please input the keywords of certain topic, use "," to connect: '
        )
    ...
    def settings_():
        ...
        * whether hide the password while typing it.
        ...
        if index == 1:
            hide_pwd = True if get_int(
                'Whether hide password while typing (0 or 1): ', range_=(0, 1)
            ) else False
        return None

    funcs = [
        quit_, Diary.new_page, view_, Diary.del_page, edit_,
        Diary.save, sync_, import_, Diary.set_password, recall_, settings_]
    # functions to execute later
    ...
    while True:
        ...
        print(
            ...
            '10. Settings\n'
            '0. Quit' %
            Diary.current_file.replace('files/', '').replace('.dr', ''))
        mode = get_int('#: ', range_=range(11))
        print('=' * 50)

        funcs[mode]()  # call the functions defined in line 174
